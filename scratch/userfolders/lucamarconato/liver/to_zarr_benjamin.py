...
data_path = "../../../datasets/liver/"
assert "Resolve" in os.listdir(data_path)
##
...
def create_points_element(path: str) -> pa.Table:
    ...
    # dummy = sd.PointsModel.parse(np.zeros(shape=(2, 2)), transform=composed)
...
data_path = pathlib.Path(data_path) / 'ResolveData_HCA'
...
for o in organisms:
    ...
    for filename in os.listdir(data_path / o):
        if filename.endswith(".txt"):
            ...
            # TODO: remove dummy patch
            dummy = sd.Image2DModel.parse(np.zeros(shape=(2, 2, 2)), dims=("c", "y", "x"),
                                          multiscale_factors=multiscale_factors, transform=composed)
            correct_transform = sd.get_transform(dummy)
            for axis in correct_transform.output_coordinate_system._axes:
                if axis.name != "c":
                    axis.unit = "micrometer"
